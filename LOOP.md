...
> **Hot-thread poll (arm only during degradation / when a thread is hot).** When the daemon is dropping events OR a thread is hot/time-sensitive, arm a dedicated short-interval (~15m) read-back poll for that thread (e.g. via a cron) that runs independently of this hourly loop. **Currently NONE active.** Note (2026-06-29): **XMTP *sending* is blocked by a per-IP rate limit on `GetIdentityUpdatesV2`** (reads still work; Discord/Telegram fine) — send-path-only, so the hourly read sweep still covers XMTP and no poll is needed unless an XMTP thread goes hot while sends stay blocked. The SX Ledger thread (`metro://discord/d0/1517956186929102869`) is dormant. Re-arm if inbound drops recur or a thread heats up.
