...
- **Metro daemon reliability — DEGRADED BOTH DIRECTIONS (2026-06-27).** Inbound DROPS (count-sweeps arriving with gaps) AND outbound send/react hitting **300s timeouts repeatedly** at the same time. **A daemon restart is the likely fix.** Read-back (PART-1 inbox sweep) still reconciles inbound; outbound needs retry/restart. See skills/metro-comms-reliability.md.
