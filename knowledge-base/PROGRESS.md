...
**Concrete next focus:** **Verify the XMTP send-path recovery once the Fly deploy lands** — confirm the cloud `tony` client boots (deployed code now references `XMTP_TONY_PRIVATE_KEY`), inbox stays at ~1/10 installations with `DERIVE_COUNT=1`, the `R152…DEF` per-IP bucket clears for the new identity, and a real send succeeds end-to-end. If it works, flip the XMTP-send STATUS item from "deploy in progress" to RECOVERED and note the loader patch is now committed/deployed (no longer deploy-drift). If it doesn't, capture the next failure layer in `skills/xmtp-multi-client-installation-cap.md`. Secondary: re-verify drifted IDs/dashboards/sources; keep STATUS/reports fresh. KB capability map stays in maintenance mode (no net-new mapping outstanding).
