...
3. **Await results** — #2125 `active_proposal_count` **live-data re-verification** (real non-empty source + active proposals; Sekhmet rejected the synthetic-row proof) → on pass, report to Sekhmet + retarget UI PR #2195 to master after #2125 merges. And **Less's #2185 rebase decision** (cherry-pick UI commit `66db2dfd` only) — execute on confirm.
