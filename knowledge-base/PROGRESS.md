...
## TODO
- **`knowledge-base/channels/`** — not created. Enumerate every channel/conversation Tony is in (Discord/Telegram/XMTP): id, name, station, purpose, participants (handle/id/role/trust/prefix). One file per major channel + a navigable index. Mark trusted-5 explicitly.
...
Build `knowledge-base/channels/` index (enumerate channels + users via metro `read` / `list_accounts` / `group_info`), AND start the per-MCP `knowledge-base/mcp/` files beginning with **metro**, **Snapshot**, and **Better Stack** using SAFE read-only calls only (e.g. `snapshot-whoami`, `uptime_list_monitors`, `telemetry_list_sources`). Document write/mutating tools from their schemas without invoking them.
