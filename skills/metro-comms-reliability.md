...
1. **Send/react latency (false-error-but-sent):** a metro write verb (`send`/`reply`/`react`) can take 300s and hit the MCP idle-timeout ERROR even though the write **actually SUCCEEDED server-side**. **Do NOT blindly retry** — verify by reading the channel back, then resend only if genuinely absent. (Recurs across sessions.) **Treat a write timeout as "unknown, probably delivered" — NOT "failed."**
   - **Confirmed 2026-06-29:** three consecutive `reply`/`send` calls to a Discord channel all returned the 300s idle-timeout ERROR, but a subsequent `read` showed **all three messages posted**. A blind retry on each would have **double/triple-posted (spam)**. Lesson: after any metro write times out, FIRST `read` the channel to check whether it landed before resending. Reactions are cheap to re-verify too (`read` then `react` only if absent). See `knowledge-base/mcp/metro.md` (daemon write-verb timeout semantics).
...
**2026-06-29 — `read`-verb-only degradation (selective):** the metro `read` verb timed out (300s) repeatedly across a tick while `react`/`reply` AND inbound SSE streaming kept working. So degradation isn't always all-or-nothing — history sweeps (`read`) can be down while live inbound + replies are fine. Mitigation is the same: a client-side **`/mcp` reconnect** is the lever; in the meantime lean on live inbound for new messages since back-sweeps are the degraded path.
